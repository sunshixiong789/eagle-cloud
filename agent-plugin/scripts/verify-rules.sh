...
for c in LogMask SecurityUtils EagleUser DistributedLock CacheProtectionUtil \
         ...
         TenantContextHolder RedisRateLimiter EncryptedStringConverter DataScope \
         ContextPropagationConfig AbstractRocketMqListener BusinessMetrics; do
...
check_key() {
  local key="$1"
  # 支持 @ConditionalOnProperty(prefix=..., name=...) 的拆写形式
  local prefix="${key%.*}"
  ...
  if grep -rqF "$key" --include='*.java' --include='*.yml' eagle-services eagle-starter 2>/dev/null \
    || grep -rq "prefix = \"$prefix\"" --include='*.java' eagle-starter 2>/dev/null \
