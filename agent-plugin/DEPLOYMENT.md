...
```bash
codex plugin marketplace add https://gitlab.your-domain.com/eagle/eagle-cloud.git
...
# Codex CLI
codex plugin marketplace add https://gitea.your-domain.com/eagle/eagle-cloud.git
codex plugin install eagle-cloud@eagle-cloud
```
...
validate:
  ...
  steps:
    ...
    - chmod +x ./agent-plugin/scripts/sync.sh
    - bash ./agent-plugin/scripts/sync.sh
    - |
      if ! git diff --quiet agent-plugin/; then
        echo "❌ agent-plugin/ is OUT OF SYNC"
        git diff agent-plugin/
        exit 1
      fi
    - jq -e '.name and .version' agent-plugin/.claude-plugin/plugin.json
    - jq -e '.name and .version' agent-plugin/.codex-plugin/plugin.json
...
- 选 `agent-plugin/` 目录或建立独立 `docs/` 分支
...
```bash
# 用 git subtree 抽取 agent-plugin/ 到新仓库
cd /path/to/eagle-cloud
git subtree split --prefix=agent-plugin -b plugin-only
...
# Codex CLI
codex plugin marketplace add https://gitlab.your-domain.com/eagle/eagle-cloud-plugin.git
codex plugin install eagle-cloud@eagle-cloud
```
    ...
    steps:
      ...
      - name: Push subtree to plugin repo
        ...
        run: |
          ...
          git subtree push --prefix=agent-plugin "$PLUGIN_REPO" main
