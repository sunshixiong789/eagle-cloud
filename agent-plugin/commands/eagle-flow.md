...
`eagle-feature-flow` 改为**仅手动触发**,模型不再因为听到"新功能/加一个模块/重构 X"等
普通需求描述就自动激活 flow。要启动 6 阶段流程,**必须**使用以下任一显式入口:

| 方式            | 例子                                                   |
|---------------|------------------------------------------------------|
| Slash command | `/eagle-flow 用户积分系统` / `/eagle-flow`(空参)             |
| 自然语言显式短语      | `按 eagle flow 走`、`启动 eagle flow`、`走 eagle flow`     |
| 跨 session 恢复  | `/eagle-flow continue` 或 `继续 eagle flow`             |

普通的"我要做一个用户积分系统"这类描述会被当作常规开发请求处理,**不会**进入 6 阶段编排 ——
如果希望走 flow,请追加一句"按 eagle flow 走"或直接用 `/eagle-flow`。
