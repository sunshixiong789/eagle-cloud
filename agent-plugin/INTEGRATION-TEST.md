...
**同时适用 Claude Code 与 Codex CLI**——除安装命令外，验证步骤完全一致。
...
在 Claude Code / Codex 会话中输入：

```
/help          # Claude Code
/plugins       # Codex CLI（浏览已安装 plugin）
```

**预期**：能看到以下 6 个项目级命令：

```
/eagle-flow         启动 Eagle 端到端开发流程（6 阶段编排）
...
**预期**：能从 CLAUDE.md / AGENTS.md / plugin.json 中找到版本号 `1.9.0`
