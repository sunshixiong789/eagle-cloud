...
// ❌ 禁止：把异常拼到 message
log.error("failed: " + ex.getMessage());        // 丢堆栈
log.error("failed: {}", ex);                    // 占位符吞堆栈
```
...
inventoryFeignClient.lockStock(items);
log.info("inventory locked: items={}, costMs={}", items.size(), System.currentTimeMillis() - start);

// ✅ 警告路径 WARN：预期异常但有业务影响
log.warn("stock insufficient, fallback to backorder: orderId={}", order.getId());

// ✅ 失败路径 ERROR + 堆栈
catch (PaymentException ex) {
    log.error("payment failed: orderId={}", order.getId(), ex);
    throw PaymentErrorCode.GATEWAY_ERROR.toServiceException(ex);
}
```
...
// ❌ 完整对象 dump
log.info("order paid: {}", order);   // 噪声 + 可能含敏感字段
```
...
try {
    ...
    log.error("payment failed, orderNo={}", request.getOrderNo(), ex);
    throw PaymentErrorCode.GATEWAY_ERROR.toServiceException(ex);
}
...
```java
    ...
    log.warn("domain rule violated: {}", ex.getErrorCode(), ex);
    throw ex;
}
```
...
if (log.isDebugEnabled()) {
    log.debug("aggregate state: {}", aggregate.dumpFullState());
}
```
...
业务关键事件（订单创建、支付完成、用户注册）**通过领域事件发布**，由 `infrastructure/event/` 统一异步落库或推送 ELK，**不**靠 `log.info` 来"记录历史"。
