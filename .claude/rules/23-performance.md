...
```java
// ✅ 集中配置（在 eagle-common-starter）
@Bean("eagleTaskExecutor")
public TaskExecutor taskExecutor() {
    ThreadPoolTaskExecutor exec = new ThreadPoolTaskExecutor();
    exec.setCorePoolSize(10);
    exec.setMaxPoolSize(50);
    exec.setQueueCapacity(500);              // 有界队列
    exec.setKeepAliveSeconds(60);
    exec.setRejectedExecutionHandler(new CallerRunsPolicy());
    exec.setThreadNamePrefix("eagle-async-");
...
- **禁止**用 `@Async` 默认 `SimpleAsyncTaskExecutor`（每次 new 线程）
