---
name: eagle-redis
description: Use when working with Redis in eagle-cloud projects — Spring caching with @Cacheable, distributed locks (DistributedLock/Redisson), CacheProtectionUtil getWithMutex (4 params with Class type), bloom filter, rate limiting (token bucket / sliding window), atomic counters with CAS, delayed queue, Pub/Sub topic
---
