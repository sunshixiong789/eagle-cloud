#!/usr/bin/env bash
# sync.sh — 从 eagle-cloud 仓库源同步内容到 plugin 目录
# 运行：在仓库根目录执行 ./claude-plugin/sync.sh
...
echo "==> 同步 rules/"
rm -rf "$PLUGIN_DIR/rules"
mkdir -p "$PLUGIN_DIR/rules"
cp "$ROOT_DIR/.claude/rules"/*.md "$PLUGIN_DIR/rules/"
echo "    $(ls "$PLUGIN_DIR/rules" | wc -l | tr -d ' ') files"

echo "==> 同步 commands/"
rm -rf "$PLUGIN_DIR/commands"
mkdir -p "$PLUGIN_DIR/commands"
cp "$ROOT_DIR/.claude/commands"/*.md "$PLUGIN_DIR/commands/"
echo "    $(ls "$PLUGIN_DIR/commands" | wc -l | tr -d ' ') files"

echo "==> 生成 skills/{starter}/SKILL.md"
