...
4. **claude-plugin 知识库注入**是**强制**的,不是建议 —— Phase 2 必须读 rules,Phase 3 必须加载 starter skill
...
**claude-plugin 介入**:无。本阶段聚焦需求,不引入实现细节约束。
...
1. **`/check-arch`**(claude-plugin 提供) — Modulith 静态验证 + 模块测试 + 全量构建,3/3 全绿
...
1. 打开 `claude-plugin/rules/25-review-checklist.md`,逐项对照(命名 / 架构 / API / 数据库 / 异常 / 日志 / 并发 / 测试 / Starter / Feign / 安全 / 缓存 / 消息 / 多租户 / 配置 / 性能,共 16 大类)
...
## claude-plugin 知识库总览(供 Phase 2 / Phase 3 查阅)

| 资源类型              | 路径                                  | 用途                       |
|-------------------|-------------------------------------|--------------------------|
| **规范文档(必读)**     | `claude-plugin/rules/01-30*.md`     | Phase 2 写 plan 的约束输入     |
| **PR 自检清单**      | `claude-plugin/rules/25-review-checklist.md` | Phase 5 评审依据          |
| **Slash Commands** | `claude-plugin/commands/*.md`       | Phase 3 触发的脚手架命令         |
| **Starter Skills** | `claude-plugin/skills/eagle-*/SKILL.md` | Phase 3 写代码时按需加载         |

主索引在 `claude-plugin/CLAUDE.md`。

---

## 与 Superpowers 的边界(避免概念混淆)

| Superpowers 提供                | claude-plugin 提供                |
|-------------------------------|---------------------------------|
